...
class Markov:
        ...
        for i in range(len(self.nombrecasillas)):
            for j in range(0, 13):
                if i + j < len(self.nombrecasillas):
                    matriz[i, i + j] = round(prob_dados[j], 3)
    ...
    def calc_prob_dados() -> list:
        ...
        probabilidades = [0, 0] + probabilidades
        return probabilidades

# Crear una instancia de la clase Markov
markov = Markov()

print(f"{markov.prob_dados}\n\n{len(markov.prob_dados)}")

#Imprimir la matriz en archivo
# np.savetxt('matriz.txt', markov.prob_transicion, fmt='%f')
...
def simular_partidas():
    ...
    for _ in range(n_partidas):
        tablero = Tablero()
        jugador = Jugador("Jugador 1", tablero)
        jugador.mover()
        indice = tablero.casillas.index(jugador.posicion_actual)
        tablero.casillas[indice].veces_visitada += 1
    
    for casilla in tablero.casillas:
        print(f"{casilla.nombre} visitada {casilla.veces_visitada} veces")
